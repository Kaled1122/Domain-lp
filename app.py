...
if not OPENAI_KEY: raise ValueError("Missing OPENAI_API_KEY")
if not DB_URL: raise ValueError("Missing DATABASE_URL")
...
Create a full StanEval-aligned lesson plan with:
1. Objectives (linked to CEFR)
2. Vocabulary & grammar focus
3. Stages: Warm-Up, Practice, Production
4. Teacher/Learner roles
5. Assessment criteria under U/A/C/B domains
6. Summary table and reflection task
...
"""
        resp=client.chat.completions.create(
            model="gpt-4o-mini",temperature=0.4,timeout=45,
            messages=[{"role":"system","content":"You create professional ELT lesson plans with StanEval domains."},
                      {"role":"user","content":prompt}]
        )
        text=resp.choices[0].message.content
        return f"<html><body style='font-family:Arial;padding:20px'>{text.replace(chr(10),'<br>')}</body></html>",200,{"Content-Type":"text/html"}
    except Exception as e:
        logging.exception(e);return f"<p style='color:red'>Error: {e}</p>",500
